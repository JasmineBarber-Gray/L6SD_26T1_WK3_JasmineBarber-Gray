...
complexity_data = {
    ...
    "Sorted": [
        "O(n²)",      # Bubble (no optimization)
        "O(n)",       # Insertion best case
        "O(n²)",      # Selection
        "O(n log n)", # Merge
        "O(n log n)", # Quick
        "O(n log n)"  # Heap
...
# Save both to same Excel file using different sheets
with pd.ExcelWriter("Time_Complexities.xlsx") as writer:
